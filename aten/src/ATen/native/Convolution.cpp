  ...
  if (params.transposed) {
    ...
    if (dim == 4) {
      if (dilated) {
        ...
        if (params.use_nnpack(input)) {
#if AT_NNPACK_ENABLED()
          return std::make_tuple(
              at::_nnpack_spatial_convolution(input, weight, bias, padding),
              save1,
              save2,
              /*impl_index=*/9);
#endif
          ...
          return std::tuple_cat(
              at::thnn_conv2d_forward(
                  input, weight, kernel_size, bias, stride, padding),
              /*impl_index=*/std::make_tuple(10));
        }
      }
      ...
      /* CPU implementation has specialized MM kernels
         for non-dilated case here */
      return std::tuple_cat(
          at::thnn_conv3d_forward(
              input, weight, kernel_size, bias, stride, padding),
          /*impl_index=*/std::make_tuple(12));
    }
  }
...
at::Tensor _convolution(
    ...
    bool deterministic,
    bool cudnn_enabled) {
  return std::get<0>(at::_convolution_impl_index(
      input_r,
      weight_r,
      bias_r,
      stride_,
...
std::tuple<Tensor, Tensor, Tensor> _convolution_nogroup_impl_index_backward(
  ...
  if (impl_index == 6) {
    ...
    std::tie(grad_input, grad_weight, grad_bias) = at::conv_dilated2d_backward(
        grad,
        self,
        weight,
        kernel_size,
    ...
    std::tie(grad_input, grad_weight, grad_bias) = at::conv_dilated3d_backward(
        grad,
        self,
        weight,
        kernel_size,
...
std::tuple<Tensor, Tensor, Tensor> _convolution_impl_index_backward(
  ...
  if (impl_index == 0) {
    ...
    std::tie(grad_input, grad_weight) = at::thnn_conv_depthwise2d_backward(
        ...
        out_mask);
    grad_bias =
        grad.contiguous().view({grad.size(0), grad.size(1), -1}).sum(0).sum(1);
    ...
    std::tie(grad_input, grad_weight, grad_bias) =
        at::mkldnn_convolution_backward(
            input,
            grad,
            weight,
            params.padding,
    ...
    if (params.groups == 1) {
      std::tie(grad_input, grad_weight, grad_bias) =
          _convolution_nogroup_impl_index_backward(
              impl_index,
              input,
              grad,
              weight,
